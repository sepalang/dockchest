...
const { buildCommandInfoWithPrompt, createSimpleGuideByCommandInfo } = require('../cli-builder')
...
async function init ({ executePath }){
  ...
  await findDockchestFile(targetDir).then((dockchestPathes)=>{
    return arrayMapPromise(dockchestPathes, async (dockchestPath)=>{
      const preconfig = await readDockchestFile(dockchestPath)
      const commandInfo = await reateCommandInfo(preconfig)
      const info = createSimpleGuideByCommandInfo(commandInfo)
